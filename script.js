...
function youtubeLoopUrl(url = "", extra = "") {
  ...
  if (!id) return "";
  return `https://www.youtube.com/embed/${id}?autoplay=1&mute=1&loop=1&playlist=${id}&playsinline=1&rel=0${extra}`;
}
...
function renderHome() {
  ...
  if (latest) {
    const ann = latest[1];
    const videoSrc = ann.youtubeUrl ? youtubeLoopUrl(ann.youtubeUrl, "&enablejsapi=1") : "";
...
function openAnnouncementModal(ann) {
  ...
  if (videoSrc) {
    ...
    vidWrap.innerHTML = `<iframe class="video-frame" src="${videoSrc}" allowfullscreen allow="autoplay; encrypted-media; picture-in-picture"></iframe>`;
  } else {
    img.classList.add("hidden");
