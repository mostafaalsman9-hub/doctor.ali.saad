...
const state = {
  ...
  announcements: {},
  schedules: {},
  settings: { aboutText: "", contactText: "", backgroundUrl: "" }
};
...
function setupRealtimeListeners() {
  ...
  subscribe("settings", data => { state.settings = { aboutText: "", contactText: "", backgroundUrl: "", ...data }; renderHome(); renderAdminContent(); });
}
...
function renderHome() {
  ...
  if (latest) {
    const ann = latest[1];
    const vidId = ann.youtubeUrl ? youtubeId(ann.youtubeUrl) : "";
    latestBox.classList.remove("hidden");
    latestBox.innerHTML = `
      <div class="announcement-preview">
        ${vidId ? `
          <div class="announcement-video-wrap">
            <iframe class="video-frame announcement-autoplay"
              data-src="https://www.youtube.com/embed/${vidId}?autoplay=1&mute=1&enablejsapi=1"
              allowfullscreen allow="autoplay; encrypted-media; picture-in-picture"></iframe>
          </div>
        ` : ann.imageUrl ? `<img src="${escapeHtml(ann.imageUrl)}" alt="${escapeHtml(ann.title)}">` : ""}
        <div>
          <strong>أحدث إعلان</strong>
    ...
    box.innerHTML = entries.length ? entries.map(([, video]) => {
      ...
      return `
        <article class="item-card">
          ${id ? `<iframe class="video-frame${shortClass}" src="https://www.youtube.com/embed/${id}" allowfullscreen></iframe>` : ""}
...
function openAnnouncementModal(ann) {
  ...
  if (vidId) {
    ...
    vidWrap.innerHTML = `<iframe class="video-frame" src="https://www.youtube.com/embed/${vidId}?autoplay=1&mute=1" allowfullscreen allow="autoplay; encrypted-media; picture-in-picture"></iframe>`;
  } else {
    img.classList.toggle("hidden", !ann.imageUrl);
    if (ann.imageUrl) img.src = ann.imageUrl;
...
function renderAnnouncementsPanel() {
  return `
    <form class="form-card" id="announcementForm">
      ...
      <label>رابط الصورة (اختياري)</label><input id="announcementImage" dir="ltr" placeholder="https://...">
      <label>رابط فيديو YouTube (اختياري)</label><input id="announcementYoutube" dir="ltr" placeholder="https://www.youtube.com/watch?v=...">
...
function bindAdminForms() {
  ...
  if (announcementForm) announcementForm.addEventListener("submit", async e => {
    e.preventDefault();
    const data = { title: $("#announcementTitle").value.trim(), text: $("#announcementText").value.trim(), imageUrl: $("#announcementImage").value.trim(), youtubeUrl: $("#announcementYoutube").value.trim(), createdAt: new Date().toISOString() };
  ...
  if (settingsForm) settingsForm.addEventListener("submit", async e => {
    e.preventDefault();
    await set(ref(db, "settings"), { aboutText: $("#settingsAbout").value, contactText: $("#settingsContact").value, backgroundUrl: $("#settingsBackground").value });
...
function setupEvents() {
  ...
  $("#studentContent").addEventListener("click", event => {
    ...
    if (!card) return;
    const title = card.dataset.openAnnouncement;
    const entries = sortedEntries(state.announcements);
