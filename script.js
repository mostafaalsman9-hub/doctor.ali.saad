...
const CATEGORIES = [
  { id: "rescue", name: "إنقاذ الغرقي والاسعافات الأولية", icon: "◎", image: "./images/rescue.jpg", desc: "تعلم أساسيات وتقنيات الإنقاذ المائي." },
  { id: "instructor", name: "كورس إعداد معلم السباحة", icon: "▣", image: "./images/instructor.jpg", desc: "برنامج شامل لإعداد معلمين محترفين." },
  { id: "golden", name: "برنامج البطل الذهبي لتعليم السباحة", icon: "☆", image: "./images/golden.jpg", desc: "برنامج متكامل لتعليم السباحة." },
  { id: "general", name: "عام (للجميع)", icon: "◇", image: "./images/general.jpg", desc: "محتوى تعليمي متاح للجميع." }
];
...
function contactHref(type, value = "") {
  ...
  if (!raw) return "";
  if (type === "phone") return raw.startsWith("tel:") ? raw : `tel:${raw.replace(/[^\d+]/g, "")}`;
  if (type === "whatsapp") {
...
function contactIconsHtml() {
  const links = [
    { type: "facebook", label: "فيسبوك", value: state.settings.facebookUrl, icon: "f" },
    { type: "tiktok", label: "تيك توك", value: state.settings.tiktokUrl, icon: "♪" },
    { type: "instagram", label: "انستقرام", value: state.settings.instagramUrl, icon: "◎" },
    { type: "whatsapp", label: "واتساب", value: state.settings.whatsappUrl, icon: "☎" },
    { type: "phone", label: "اتصال هاتفي", value: state.settings.phoneNumber, icon: "✆" }
  ].map(item => ({ ...item, href: contactHref(item.type, item.value) })).filter(item => item.href);
  ...
  return `
    ...
    <div class="contact-icons">
      ${links.map(item => `
        <a class="contact-icon ${item.type}" href="${escapeHtml(item.href)}" target="_blank" rel="noopener noreferrer" aria-label="${escapeHtml(item.label)}">
          <span>${item.icon}</span>
...
function renderHome() {
  $("#publicCategories").innerHTML = CATEGORIES.map(cat => `
    <article class="category-card">
      <img class="category-img-icon" src="${escapeHtml(cat.image)}" alt="${escapeHtml(cat.name)}" />
      <h3>${cat.name}</h3>
      <p>${cat.desc}</p>
    </article>
  `).join("");
    ...
    latestBox.innerHTML = `
      ...
      <div class="home-announcements-list">
        ${latestAnnouncements.map(([, ann]) => {
          const videoSrc = ann.youtubeUrl ? youtubeLoopUrl(ann.youtubeUrl) : "";
  ...
  const contactHtml = contactIconsHtml();
  $("#aboutSection").classList.toggle("hidden", !about.trim());
  $("#contactSection").classList.toggle("hidden", !contactHtml);
        ...
        <article class="item-card">
          ${id ? `
            <div class="youtube-player-wrap${shortClass}">
              <iframe class="video-frame${shortClass}" src="${videoSrc}" allow="autoplay; encrypted-media; picture-in-picture"></iframe>
              <span class="youtube-share-blocker" aria-hidden="true"></span>
              <button class="youtube-fullscreen-btn" type="button" aria-label="ملء الشاشة">⛶</button>
            </div>
          ` : ""}
...
function openAnnouncementModal(ann) {
  ...
  if (videoSrc) {
    ...
    vidWrap.innerHTML = `
      <div class="youtube-player-wrap">
        <iframe class="video-frame" src="${videoSrc}" allow="autoplay; encrypted-media; picture-in-picture"></iframe>
        <span class="youtube-share-blocker" aria-hidden="true"></span>
        <button class="youtube-fullscreen-btn" type="button" aria-label="ملء الشاشة">⛶</button>
      </div>
    `;
...
function renderSettingsPanel() {
  return `
    <form class="form-card" id="settingsForm">
      ...
      <h3 class="settings-subtitle">روابط التواصل التي تظهر فوق الفوتر</h3>
      <label>رابط فيسبوك</label><input id="settingsFacebook" dir="ltr" value="${escapeHtml(state.settings.facebookUrl || "")}" placeholder="https://facebook.com/...">
      <label>رابط تيك توك</label><input id="settingsTiktok" dir="ltr" value="${escapeHtml(state.settings.tiktokUrl || "")}" placeholder="https://tiktok.com/@...">
      <label>رابط انستقرام</label><input id="settingsInstagram" dir="ltr" value="${escapeHtml(state.settings.instagramUrl || "")}" placeholder="https://instagram.com/...">
      <label>رقم أو رابط واتساب</label><input id="settingsWhatsapp" dir="ltr" value="${escapeHtml(state.settings.whatsappUrl || "")}" placeholder="201xxxxxxxxx أو https://wa.me/...">
      <label>رقم التواصل هاتفيا</label><input id="settingsPhone" dir="ltr" value="${escapeHtml(state.settings.phoneNumber || "")}" placeholder="+201xxxxxxxxx">
...
function bindAdminForms() {
  ...
  if (settingsForm) settingsForm.addEventListener("submit", async e => {
    ...
    await set(ref(db, "settings"), {
      aboutText: $("#settingsAbout").value,
      contactText: "",
      facebookUrl: $("#settingsFacebook").value.trim(),
      tiktokUrl: $("#settingsTiktok").value.trim(),
      instagramUrl: $("#settingsInstagram").value.trim(),
      whatsappUrl: $("#settingsWhatsapp").value.trim(),
      phoneNumber: $("#settingsPhone").value.trim(),
      backgroundUrl: $("#settingsBackground").value,
      backgroundYoutubeUrl: $("#settingsBackgroundYoutube").value.trim()
    });
