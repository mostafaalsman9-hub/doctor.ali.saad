                ...
                <div class="announcement-video-wrap">
                  <div class="youtube-player-wrap">
                    <iframe class="video-frame"
                      src="${videoSrc}"
                      allowfullscreen allow="autoplay; encrypted-media; picture-in-picture; fullscreen"></iframe>
        ...
        <article class="item-card">
          ${id ? `
            <div class="youtube-player-wrap${shortClass}">
              <iframe class="video-frame${shortClass}" src="${videoSrc}" allowfullscreen allow="autoplay; encrypted-media; picture-in-picture; fullscreen"></iframe>
              <span class="youtube-share-blocker" aria-hidden="true"></span>
              <button class="youtube-fullscreen-btn" type="button" aria-label="ملء الشاشة">⛶</button>
            </div>
          ` : ""}
...
function openAnnouncementModal(ann) {
  ...
  if (videoSrc) {
    ...
    vidWrap.innerHTML = `
      <div class="youtube-player-wrap">
        <iframe class="video-frame" src="${videoSrc}" allowfullscreen allow="autoplay; encrypted-media; picture-in-picture; fullscreen"></iframe>
        <span class="youtube-share-blocker" aria-hidden="true"></span>
        <button class="youtube-fullscreen-btn" type="button" aria-label="ملء الشاشة">⛶</button>
      </div>
    `;
...
function closeYoutubeSiteFullscreen() {
  const overlay = $("#youtubeSiteFullscreen");
  if (overlay) overlay.remove();
  document.body.classList.remove("no-scroll");
}

function openYoutubeSiteFullscreen(src = "", isShort = false) {
  if (!src) return;
  closeYoutubeSiteFullscreen();
  const videoSrc = setYoutubeParam(setYoutubeParam(src, "autoplay", "1"), "playsinline", "1");
  document.body.insertAdjacentHTML("beforeend", `
    <div class="youtube-site-fullscreen" id="youtubeSiteFullscreen" role="dialog" aria-modal="true" aria-label="مشاهدة الفيديو بملء الشاشة">
      <div class="youtube-site-fullscreen-box${isShort ? " short" : ""}">
        <iframe class="youtube-site-fullscreen-frame${isShort ? " short" : ""}"
          src="${escapeHtml(videoSrc)}"
          allowfullscreen
          allow="autoplay; encrypted-media; picture-in-picture; fullscreen"></iframe>
        <button class="youtube-site-fullscreen-close" type="button" aria-label="إغلاق ملء الشاشة">×</button>
      </div>
    </div>
  `);
  document.body.classList.add("no-scroll");
...
async function toggleYoutubeFullscreen(button) {
  ...
  openYoutubeSiteFullscreen(src, player.classList.contains("short") || iframe?.classList.contains("short"));
}
...
function setupEvents() {
  ...
  document.addEventListener("click", event => {
    if (event.target.closest(".youtube-site-fullscreen-close") || event.target.id === "youtubeSiteFullscreen") {
      closeYoutubeSiteFullscreen();
    }
  });

  document.addEventListener("keydown", event => {
    if (event.key === "Escape") closeYoutubeSiteFullscreen();
  });
